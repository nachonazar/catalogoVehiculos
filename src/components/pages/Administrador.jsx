...
const Administrador = () => {
  ...
  const vehiculosDisponibles = listaVehiculos.filter(v => v.disponible).length;
  const vehiculosVendidos = listaVehiculos.length - vehiculosDisponibles;

  return (
    <div className="bg-neutral-bg text-on-surface font-body-md min-h-screen flex">
      
      {/* PANEL LATERAL IZQUIERDO (Forzado al azul oscuro de Stich #051125) */}
      <nav className="hidden lg:flex flex-col h-screen w-64 fixed left-0 top-0 bg-[#051125] shadow-lg z-50 p-4 text-on-primary font-body-md transition-all duration-300">
        <div className="mb-8 px-4 py-2">
          {/* no-underline evita el subrayado */}
          <Link to="/" className="font-headline-md text-headline-md font-bold text-on-primary tracking-tight hover:text-secondary-fixed transition-colors no-underline">
            LUXE AUTO
          </Link>
          <p className="text-on-primary/70 text-sm mt-1">Panel de Administración</p>
        </div>

        <ul className="flex flex-col gap-2 flex-grow list-none p-0">
          <li>
            <Link to="/administrador" className="flex items-center gap-3 px-4 py-3 bg-secondary-container text-on-secondary-container rounded-lg shadow-sm no-underline">
              <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>directions_car</span>
              <span className="font-semibold">Inventario</span>
            </Link>
          </li>
          <li>
            <a href="#" className="flex items-center gap-3 px-4 py-3 text-on-primary/70 hover:text-on-primary hover:bg-primary-container transition-colors duration-200 rounded-lg group no-underline">
        ...
        <div className="mt-auto border-t border-primary-container pt-4">
          <Link to="/administrador/crear" className="w-full mb-4 bg-secondary text-white py-2 px-4 rounded-lg font-semibold shadow-sm hover:bg-secondary/90 transition-colors flex items-center justify-center gap-2 no-underline">
            <span className="material-symbols-outlined text-sm">add</span> Nuevo Vehículo
          </Link>
          <ul className="flex flex-col gap-1 list-none p-0">
            <li>
              <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-on-primary/70 hover:text-on-primary hover:text-error-container transition-colors bg-transparent border-0 text-left">
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <header className="fixed top-0 right-0 w-full lg:w-[calc(100%-16rem)] h-16 bg-surface shadow-sm z-40 flex justify-between items-center px-4 md:px-gutter transition-all">
          <div className="flex items-center gap-4">
            <h2 className="font-headline-md text-[20px] text-primary font-semibold tracking-tight">Control de Inventario</h2>
          </div>
        ...
        <main className="flex-1 mt-16 p-4 md:p-gutter w-full max-w-container-max mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-stack-lg gap-4 pt-4">
            <div>
              <h2 className="font-headline-lg text-headline-lg text-primary tracking-tight">Inventario de Vehículos</h2>
              <p className="text-on-surface-variant mt-1 font-body-md">Gestiona, actualiza y monitorea tu flota actual.</p>
            </div>
            <Link to="/administrador/crear" className="lg:hidden bg-primary text-white px-6 py-3 rounded-lg font-semibold shadow-md hover:bg-primary/90 transition-all flex items-center gap-2 no-underline">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-stack-lg">
            <div className="bg-surface-container-lowest border border-surface-variant p-6 rounded-xl flex items-center gap-4 shadow-sm">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                <span className="material-symbols-outlined text-[28px]" style={{fontVariationSettings: "'FILL' 1"}}>directions_car</span>
              </div>
              <div>
                <p className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider">Vehículos Totales</p>
                <p className="font-headline-lg text-headline-lg text-primary mt-1">{listaVehiculos.length}</p>
              </div>
            </div>
            ...
            <div className="bg-surface-container-lowest border border-surface-variant p-6 rounded-xl flex items-center gap-4 border-l-4 border-l-success-green shadow-sm">
              ...
              <div>
                <p className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider">Disponibles</p>
                <p className="font-headline-lg text-headline-lg text-primary mt-1">{vehiculosDisponibles}</p>
              </div>
            </div>
            ...
            <div className="bg-surface-container-lowest border border-surface-variant p-6 rounded-xl flex items-center gap-4 border-l-4 border-l-surface-tint shadow-sm">
              <div className="w-12 h-12 rounded-full bg-surface-tint/10 flex items-center justify-center text-surface-tint">
                <span className="material-symbols-outlined text-[28px]" style={{fontVariationSettings: "'FILL' 1"}}>sell</span>
              </div>
              <div>
                <p className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider">Vendidos</p>
                <p className="font-headline-lg text-headline-lg text-primary mt-1">{vehiculosVendidos}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-surface-container-lowest rounded-xl overflow-hidden shadow-sm border border-surface-variant">
            ...
            <div className="p-4 border-t border-surface-variant bg-surface/50 flex justify-between items-center">
              <span className="text-sm text-on-surface-variant hidden md:block">Página {page} de {totalPages}</span>
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 flex items-center justify-center rounded font-medium transition-colors ${
                      page === i + 1 ? "bg-primary text-white shadow-sm border-primary" : "border border-surface-variant text-on-surface-variant hover:bg-surface-variant"
                    }`}
                  >
                    {i + 1}
                  </button>
