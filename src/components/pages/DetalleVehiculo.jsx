...
const DetalleVehiculo = () => {
  ...
  const specs = [
    ...
    { icon: "speed", label: "Kilometraje", value: `${vehiculo?.km?.toLocaleString("es-AR")} km` },
  ];
  ...
  return (
    <main className="pt-[88px] pb-16 container-app">
      {/* Breadcrumb */}
      <nav aria-label="Breadcrumb" className="mb-6 flex items-center flex-wrap gap-1.5 text-sm text-on-surface-variant">
        <Link to="/" className="hover:text-on-surface no-underline transition-colors">Inicio</Link>
        <span className="material-symbols-outlined text-[16px]">chevron_right</span>
        <span>{vehiculo.categoria || "Vehículos"}</span>
        <span className="material-symbols-outlined text-[16px]">chevron_right</span>
        <span className="text-on-surface font-medium capitalize">{vehiculo.marca} {vehiculo.modelo}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="card overflow-hidden">
            <div
              className="relative w-full aspect-[4/3] md:aspect-video bg-surface-container cursor-zoom-in group"
            ...
            {vehiculo.imagenes?.length > 1 && (
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 p-4 border-t border-outline-variant">
                {vehiculo.imagenes.map((img, index) => (
                  ...
                  >
                    <img className="w-full h-full object-cover" src={img} alt={`Miniatura ${index + 1}`} />
                  </button>
                ))}
              </div>
            )}
          ...
          <section className="card p-6 md:p-8">
            <h2 className="font-heading text-lg font-semibold text-on-surface mb-5">Especificaciones</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {specs.map((spec) => (
                <div key={spec.label} className="flex flex-col gap-2 p-4 rounded-xl bg-surface-container-low border border-outline-variant">
                  <span className="material-symbols-outlined text-secondary text-[22px]">{spec.icon}</span>
                  <span className="text-label !text-[10px]">{spec.label}</span>
                  <span className="text-sm font-semibold text-on-surface">{spec.value}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          {/* Description */}
          <section className="card p-6 md:p-8">
            <h2 className="font-heading text-lg font-semibold text-on-surface mb-4">Descripción</h2>
          ...
          <div className="sticky top-[88px] flex flex-col gap-4">
            <div className="card p-6 flex flex-col gap-5">
              <div>
                ...
                <h1 className="font-heading text-2xl md:text-3xl font-bold text-on-surface capitalize leading-tight mb-3">
                  {vehiculo.marca} {vehiculo.modelo}
                </h1>
                <p className="font-heading text-3xl md:text-4xl font-bold text-on-surface tracking-tight">
                  ${vehiculo.precio?.toLocaleString("es-AR")}
                </p>
                <p className="text-label !normal-case !tracking-normal mt-1">Precio al contado</p>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="btn w-full no-underline !py-3.5 text-sm font-medium text-white border-0 shadow-md hover:shadow-lg"
            ...
            <div className="card p-6 hidden md:block">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-medium text-on-surface">Ignacio Nazar</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-secondary/10 flex items-center justify-center">
                    <span className="material-symbols-outlined text-secondary text-[18px]">location_on</span>
                  </div>
                  <p className="text-sm text-on-surface-variant">Tucumán, Argentina</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-secondary/10 flex items-center justify-center">
                  ...
                  <p className="text-sm text-on-surface-variant">
                    <span className="font-medium text-success-green">Disponible</span> · Lun a Sáb
                  </p>
                </div>
              </div>
            </div>
      ...
      {showModal && (
        ...
        >
          <button
            type="button"
            aria-label="Cerrar"
            className="absolute top-5 right-5 w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-white/20 transition-colors"
            onClick={() => setShowModal(false)}
          >
            <span className="material-symbols-outlined text-[24px]">close</span>
          </button>

          <div className="absolute top-5 left-5 px-3 py-1.5 rounded-lg bg-white/10 text-white/90 text-sm font-medium">
          ...
          {vehiculo.imagenes?.length > 1 && (
            <button
              ...
              className="absolute left-4 md:left-8 w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/20 transition-colors"
              onClick={prevImg}
            >
          ...
          {vehiculo.imagenes?.length > 1 && (
            <button
              ...
              className="absolute right-4 md:right-8 w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/20 transition-colors"
              onClick={nextImg}
            >
